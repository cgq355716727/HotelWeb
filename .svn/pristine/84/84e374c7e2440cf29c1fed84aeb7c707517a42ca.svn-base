function tabChange(key){
	if(key==1){
		document.getElementById("continentList1").style.display="block";
		document.getElementById("tit1").className="on";
		document.getElementById("continentList2").style.display="none";
		document.getElementById("tit2").className="off";
		document.getElementById("continentList3").style.display="none";
		document.getElementById("tit3").className="off";
	}else if(key==2){
		document.getElementById("continentList2").style.display="block";
		document.getElementById("tit2").className="on";
		document.getElementById("continentList1").style.display="none";
		document.getElementById("tit1").className="off";
		document.getElementById("continentList3").style.display="none";
		document.getElementById("tit3").className="off";
	}
	else if(key==3){
		document.getElementById("continentList3").style.display="block";
		document.getElementById("tit3").className="on";
		document.getElementById("continentList1").style.display="none";
		document.getElementById("tit1").className="off";
		document.getElementById("continentList2").style.display="none";
		document.getElementById("tit2").className="off";
...
//订单核对-支付方式-使用积分担保延迟入住
function checkPayMethord(type){
	if(type!=2) $("#pay_by_points").hide();
	if(type==2||type==1 || type==111){
		if(type==2)$("#pay_by_points").show();
		$("#btnpaynext")[0].className="submit";
	}else $("#btnpaynext")[0].className="next";
}
...
//按城市名称提交地图查询
function citymap(){
	if($('#mapcity').val() == ''||$('#mapcity').val() == '其他城市') {alert("请输入城市"); return;}
	window.open("map.php?city=" + encodeURI($('#mapcity').val()));
}

//信用卡验证
var pc_ret;
function validateCard(obj,type){
	//(type=1，按卡号来验证，2按银行卡名称验证)
	var ov = obj.value;
	...
	$.ajax({
		type: "GET",
		url: "check_creditcard.php",
		data: "card="+ov+"&type="+type+"&bank="+bank,
		dataType:"json",
		success: function(json){
			if(json.code==1){
				$("#div_check_credit").html("<img src='http://img1.7daysinn.cn/images/checked.gif'/>");
			}else{
				$("#div_check_credit").html("<img src='http://img1.7daysinn.cn/images/unchecked.gif'/>"+json.data);
			}
			pc_ret = json.code;
		}
	});
...
function preLoadImages()
{
	...
	imagesArray[0] = 'http://img1.7daysinn.cn/images/menu_1r.gif';
	imagesArray[1] = 'http://img1.7daysinn.cn/images/menu_2r.gif';
	imagesArray[2] = 'http://img1.7daysinn.cn/images/menu_3r.gif';
	imagesArray[3] = 'http://img1.7daysinn.cn/images/menu_4r.gif';
	imagesArray[4] = 'http://img1.7daysinn.cn/images/menu_5r.gif';
	imagesArray[5] = 'http://img1.7daysinn.cn/images/menu_6r.gif';
	imagesArray[6] = 'http://img1.7daysinn.cn/images/menu_7r.gif';
	imagesArray[7] = 'http://img1.7daysinn.cn/images/menu_8r.gif';
	...
	if(ad_arr != undefined && ad_arr.length >0)
	{
		...
		if(ad_new_arr != undefined && ad_new_arr.length >0)
		{
			...
			if(tmp_ad[2] != undefined && tmp_ad[3] != undefined)
			{
				...
				var ad_src ="<div class='blank10'></div><div><a href='" + tmp_ad[3] + "' target='_blank'>";
				ad_src +="<img src='" + pic_domain_url + tmp_ad[2] + "' width='218px' "+ " height='82px' border='0'></a></div>";
